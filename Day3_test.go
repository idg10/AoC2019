...
func TestParseWireSegment(t *testing.T) {
	cases := []struct {
		text    string
		segment WireSegment
	}{
		{"L1", WireSegment{DirectionLeft, 1}},
		{"L99", WireSegment{DirectionLeft, 99}},
		{"U42", WireSegment{DirectionUp, 42}},
		{"D123", WireSegment{DirectionDown, 123}},
	}
	for _, c := range cases {
		s := parsec.NewScanner([]byte(c.text))
		node, _ := YSegment(s)
		result := node.(WireSegment)
		if result.direction != c.segment.direction {
			t.Errorf("Parse segment '%v', got %v, want %v", c.text, result, c.segment)
		}
	}
}

func TestParseWireList(t *testing.T) {
	cases := []struct {
		text     string
		...
		{
			"L1,L99,U42,D123",
			[]WireSegment{WireSegment{DirectionLeft, 1}, WireSegment{DirectionLeft, 99}, WireSegment{DirectionUp, 42}, WireSegment{DirectionDown, 123}},
		},
		{
			"R1122,D9",
			[]WireSegment{WireSegment{DirectionRight, 1122}, WireSegment{DirectionDown, 9}},
		},
		{
			"",
			[]WireSegment{},
	...
	for _, c := range cases {
		...
		node, _ := YWirePath(s)
		result := node.([]WireSegment)
		if !reflect.DeepEqual(result, c.segments) {
			t.Errorf("Parse path '%v', got %v, want %v", c.text, result, c.segments)
		}
	}
}

func TestParseWireLists(t *testing.T) {
	cases := []struct {
		text     string
		segments [][]WireSegment
	}{
		{
			"R1122,D9",
			[][]WireSegment{
				{WireSegment{DirectionRight, 1122}, WireSegment{DirectionDown, 9}},
			},
		},
		{
			// Plain LF
			"L1,L99,U42,D123\nR1122,D9",
			[][]WireSegment{
				{WireSegment{DirectionLeft, 1}, WireSegment{DirectionLeft, 99}, WireSegment{DirectionUp, 42}, WireSegment{DirectionDown, 123}},
				{WireSegment{DirectionRight, 1122}, WireSegment{DirectionDown, 9}},
			},
		},
		{
		...
		{
			// Plain CR (not very common these days)
			"L1,L99,U42,D123\rR1122,D9",
			[][]WireSegment{
				{WireSegment{DirectionLeft, 1}, WireSegment{DirectionLeft, 99}, WireSegment{DirectionUp, 42}, WireSegment{DirectionDown, 123}},
				[]WireSegment{WireSegment{DirectionRight, 1122}, WireSegment{DirectionDown, 9}},
			},
		},
		// I was going to test with blank line too but for some reason those appear to be collapsed completely
		// and we don't see the empty array we'd expect. It's as though they're not even present. Not sure
		// why.
	}
	for _, c := range cases {
		s := parsec.NewScanner([]byte(c.text))
...
func TestDay3Part1(t *testing.T) {
	...
	for _, c := range cases {
		result := Day3Part1([]byte(c.text))
		if result != c.closest {
			t.Errorf("Input '%v', got %v, want %v", c.text, result, c.closest)
		}
	}
}
